...
async function followers(){
  ...
  const followers = followerIds.map(id => fetchUserData(id))
  const followersData = await Promise.all(followers)
  document.querySelector(".followers").innerHTML = followersData.map(
    follower => followerHtml(follower)
  ).join('')
}

function followerHtml(follower){
  return `<div class="profile">
  <img class="profile__avatar" src=${follower.avatar}>
  <div class="profile__info">
    <p class="profile__username">${follower.username}</p>
    <p class="profile__bio">${follower.bio}</p>
  </div>
